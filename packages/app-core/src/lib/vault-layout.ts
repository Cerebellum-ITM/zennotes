 ...
 * (date-pattern) + `titlePattern` shape so existing vaults keep the exact same
 * daily-note paths without any reconfiguration. Returns null when there is no
 * legacy `pathFormat` to migrate.
 */
function legacyDailyPathFormatToPattern(
  normalizedDirectory: string,
  pathFormat: string | null | undefined
): { directory: string; titlePattern: string } | null {
  const raw = (pathFormat ?? '')
    .trim()
    .replace(/^\/+|\/+$/g, '')
    .replace(/\.md$/i, '')
  if (!raw) return null
  // Uppercase moment tokens -> Unicode tokens. Month tokens (M/MM/MMM/MMMM) are
  // already shared between the two dialects; only year (Y->y) and day (D->d)
  // need lowercasing. Applied to the pattern parts only, never the base dir.
...
export function normalizeVaultSettings(
  ...
  // Back-compat (personal fork): translate a legacy `dailyNotes.pathFormat` into
  // the current directory + titlePattern shape so existing vaults keep identical
  // daily-note paths. Skipped once an explicit `titlePattern` is set.
  const legacyDaily = settings?.dailyNotes?.titlePattern
    ? null
    : legacyDailyPathFormatToPattern(
        dailyDirectory,
        (settings?.dailyNotes as { pathFormat?: string } | undefined)?.pathFormat
      )
  ...
  return {
    ...
    dailyNotes: {
      ...
      directory: legacyDaily?.directory ?? dailyDirectory,
      titlePattern:
        legacyDaily?.titlePattern ??
        normalizeDailyNoteTitlePattern(settings?.dailyNotes?.titlePattern),
