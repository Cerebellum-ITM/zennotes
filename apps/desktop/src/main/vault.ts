...
/**
 * Back-compat for the personal fork: older settings stored the daily-note layout
 * as a single `dailyNotes.pathFormat` (moment-style, relative to `directory`,
 * e.g. `YYYY/MM-MMMM/DD-MM-YYYY`). Translate it into the current `directory`
 * (date-pattern) + `titlePattern` shape so existing vaults keep identical
 * daily-note paths. Returns null when there is no legacy `pathFormat`. Mirrors
 * app-core's `legacyDailyPathFormatToPattern`.
 */
function legacyDailyPathFormatToPattern(
  normalizedDirectory: string,
  pathFormat: unknown
): { directory: string; titlePattern: string } | null {
  if (typeof pathFormat !== 'string') return null
  const raw = pathFormat.trim().replace(/^\/+|\/+$/g, '').replace(/\.md$/i, '')
  if (!raw) return null
  // Only year (Y->y) and day (D->d) tokens differ in case; month tokens match.
  const toUnicodeTokens = (s: string): string =>
    s
      .replace(/YYYY/g, 'yyyy')
      .replace(/YY/g, 'yy')
      .replace(/DD/g, 'dd')
  ...
  const folderPart = lastSlash >= 0 ? raw.slice(0, lastSlash) : ''
  const filePart = lastSlash >= 0 ? raw.slice(lastSlash + 1) : raw
  const directory = [normalizedDirectory.replace(/\/+$/, ''), toUnicodeTokens(folderPart)]
    .filter(Boolean)
    .join('/')
  return { directory, titlePattern: toUnicodeTokens(filePart) }
}
...
function normalizeVaultSettings(
  ...
  const candidate = value as {
    ...
    dailyNotes?: {
      ...
      templateId?: unknown
      // Back-compat: the personal fork's legacy single-field daily layout.
      pathFormat?: unknown
  ...
  const legacyDaily = candidate.dailyNotes?.titlePattern
    ? null
    : legacyDailyPathFormatToPattern(
        normalizeDailyNotesDirectory(candidate.dailyNotes?.directory),
        candidate.dailyNotes?.pathFormat
      )
  return {
    ...
    dailyNotes: {
      ...
      directory:
        legacyDaily?.directory ?? normalizeDailyNotesDirectory(candidate.dailyNotes?.directory),
      titlePattern:
        legacyDaily?.titlePattern ??
        normalizeDailyNoteTitlePattern(candidate.dailyNotes?.titlePattern),
...
function searchableFolderForRelPath(
  relPath: string,
  settings?: VaultSettings | null
): NoteFolder | null {
  ...
  const folder = noteFolderFromRelPath(normalized, settings)
  ...
  const walk = async (
    ...
    for (const entry of entries) {
      ...
      if (childReal !== null) {
        ...
        // archive, Daily Notes, user folders) is searchable. `settings` carries
        // the custom on-disk folder paths (#115) through to the classifier.
        const relDir = toPosix(path.relative(root, full))
        if (searchableFolderForRelPath(`${relDir}/`, settings) === null) continue
      ...
      const folder = searchableFolderForRelPath(toPosix(path.relative(root, full)), settings)
      if (!folder) continue
...
async function collectRipgrepSearchCandidates(
  ...
  for (const line of stdout.split(/\r?\n/)) {
    ...
    const folder = searchableFolderForRelPath(relPath, settings)
    if (!folder) continue
    candidates.push({
      path: relPath,
