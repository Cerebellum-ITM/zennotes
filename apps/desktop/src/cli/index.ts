...
async function main(argv: string[]): Promise<number> {
  ...
  // `icons *` reads and writes `.zennotes/icons/` on disk, so it takes a local
  // vault root rather than the VaultBackend the generic dispatch hands out. A
  // remote vault has no local icons directory to touch.
  if (command === 'icons') {
    const iconsDispatch: Record<string, (vault: string, args: ParsedArgs) => Promise<void>> = {
      'icons list': cmdIconsList,
      'icons set': cmdIconsSet,
      'icons clear': cmdIconsClear
    }
    const iconsHandler = iconsDispatch[key]
    if (iconsHandler) {
      const target = await resolveTarget(parsed)
      if (target.kind === 'remote') {
        throw new Error(
          'zn icons works on the vault’s local `.zennotes/icons/` directory, so it needs a local vault.'
        )
      }
      await iconsHandler(target.root, parsed)
      return 0
    }
  }
